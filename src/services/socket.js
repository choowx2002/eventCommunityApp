...
const SOCKET_URI = 'http://192.168.1.4:3000';// check your own ipv4 and port
const SOCKET_URI_Ethernet = 'http://192.168.176.13:5000';

let socket;

export const subscribe_notification = events => {
  return new Promise((resolve, reject) => {
    try {
      // Emit an event to subscribe to notifications
      socket.emit('subscribe_notification', events, response => {
        if (response.success) {
...
export const unsubscribe_notification = id => {
  return new Promise((resolve, reject) => {
    try {
      // Emit an event to subscribe to notifications
